    ...
    return (
        <div {...calendarProps} className='space-y-4'>
            <div className='relative flex items-center justify-center pt-1'>
                <Button {...prevButtonProps} ref={prevButtonRef} variant={'outline'} className={cn('absolute left-1 h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100')}>
                    <ChevronLeftIcon className='w-4 h-4' />
                </Button>
                <div className='text-sm font-medium'>{title}</div>
                <Button {...nextButtonProps} ref={nextButtonRef} variant={'outline'} className={cn('absolute right-1 h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100')}>
...
function CalendarCell({ state, date }: CalendarCellProps) {
    ...
    return (
        <td {...cellProps} className={cn(cellProps.className, 'relative p-0 text-center text-sm focus-within:relative focus-within:z-20 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md')}>
            <Button {...buttonProps} type='button' variant={'ghost'} ref={ref} className={cn(buttonProps.className, 'h-9 w-9', isToday ? 'bg-accent text-accent-foreground' : '', isSelected ? 'bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground focus:bg-primary focus:text-primary-foreground' : '', isOutsideVisibleRange ? 'text-muted-foreground opacity-50' : '', isDisabled ? 'text-muted-foreground opacity-50' : '')}>
                {formattedDate}
            </Button>
        </td>
    );
